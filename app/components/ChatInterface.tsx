...
function getResponse(message: string): string {
  ...
  if (lower.includes("invest") || lower.includes("stock") || lower.includes("portfolio")) return DEMO_RESPONSES.invest;
  if (lower.includes("retire") || lower.includes("retirement")) return DEMO_RESPONSES.retire;
  if (lower.includes("save") || lower.includes("saving") || lower.includes("budget")) return DEMO_RESPONSES.save;
  return DEMO_RESPONSES.default;
}

export default function ChatInterface() {
  const [messages, setMessages] = useState<Message[]>([{ id: "welcome", role: "assistant", content: "Hello! I'm your AI Wealth Advisor. I can help with investment strategies, retirement planning, budgeting, and more. What financial topic would you like to explore today?" }]);
        ...
        <div className="rounded-2xl border border-gray-800 bg-gray-900/70 overflow-hidden shadow-2xl shadow-black/50">
          <div className="flex items-center gap-3 border-b border-gray-800 px-6 py-4">
            <div className="relative">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-gold-400 to-gold-600 flex items-center justify-center"><span className="text-gray-950 font-bold text-sm">AI</span></div>
              <span className="absolute bottom-0 right-0 w-3 h-3 rounded-full bg-emerald-500 border-2 border-gray-900" />
            </div>
            <div><p className="font-semibold text-sm">AI Wealth Advisor</p><p className="text-xs text-emerald-400">Online \u2014 Ready to help</p></div>
          ...
          <div className="h-[400px] overflow-y-auto p-6 space-y-4">
            ...
            {isTyping && (
              <div className="flex justify-start"><div className="bg-gray-800 rounded-2xl rounded-bl-md px-4 py-3"><div className="flex gap-1"><span className="w-2 h-2 bg-gray-500 rounded-full animate-bounce" style={{ animationDelay: "0ms" }} /><span className="w-2 h-2 bg-gray-500 rounded-full animate-bounce" style={{ animationDelay: "150ms" }} /><span className="w-2 h-2 bg-gray-500 rounded-full animate-bounce" style={{ animationDelay: "300ms" }} /></div></div></div>
            )}
            <div ref={messagesEndRef} />
          </div>
          <div className="border-t border-gray-800 p-4">
            <div className="flex gap-3">
              <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSend(); } }} placeholder="Ask about investments, retirement, savings..." className="flex-1 rounded-xl border border-gray-700 bg-gray-800 px-4 py-3 text-sm text-white placeholder-gray-500 focus:border-gold-500 focus:outline-none focus:ring-1 focus:ring-gold-500 transition-colors" />
