...
interface PlanPaymentState { method: PaymentMethod; success: boolean; }
              ...
              <div key={plan.name} className={`relative rounded-2xl border p-8 flex flex-col ${plan.popular ? "border-gold-500/50 bg-gray-900 shadow-xl shadow-gold-500/10" : "border-gray-800 bg-gray-900/50"}`}>
                ...
                <div className="mt-8 space-y-3">
                  {!isPaid ? (
                    ...
                    <>
                      <button onClick={() => setState(plan.name, { method: "paypal" })} className="w-full py-3 rounded-lg font-semibold text-sm bg-[#0070ba] hover:bg-[#005ea6] text-white flex items-center justify-center gap-2">Pay with PayPal</button>
                      <button onClick={() => setState(plan.name, { method: "cashapp" })} className="w-full py-3 rounded-lg font-semibold text-sm bg-[#00D632] hover:bg-[#00C02E] text-white flex items-center justify-center gap-2">Pay with Cash App</button>
                    </>
                  ) : state.method === "paypal" ? (
                    <div className="space-y-3">
                      <PayPalButtons style={{ layout: "vertical", color: "gold", shape: "rect", label: "subscribe" }} createOrder={((_data: unknown, actions: unknown) => (actions as any).order.create({ purchase_units: [{ amount: { value: plan.priceValue.toString() } }] })) as any} onApprove={async () => { setState(plan.name, { success: true, method: "select" }); }} />
                      <button onClick={() => setState(plan.name, { method: "select" })} className="w-full text-xs text-gray-500 hover:text-gray-300">Back to payment options</button>
                    </div>
                    ...
                    <div className="space-y-3 text-center">
                      ...
                      <button onClick={() => setState(plan.name, { success: true, method: "select" })} className="w-full py-2.5 rounded-lg font-semibold text-sm bg-[#00D632] hover:bg-[#00C02E] text-white">I have Sent the Payment</button>
                      <button onClick={() => setState(plan.name, { method: "select" })} className="w-full text-xs text-gray-500 hover:text-gray-300">Back to payment options</button>
                    </div>
                  ) : null}
                </div>
