...
export default function Pricing() {
  ...
  const getState = (name: string): PlanPaymentState => paymentStates[name] || { method: "select", success: false };
  const setState = (name: string, patch: Partial<PlanPaymentState>) =>
    setPaymentStates((prev) => ({ ...prev, [name]: { ...getState(name), ...patch } }));
    ...
    <section id="pricing" className="py-24 px-6">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="mt-4 text-gray-400 max-w-2xl mx-auto">Choose the plan that fits your wealth-building journey.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {plans.map((plan) => {
            const isPaid = plan.priceValue > 0;
            const state = getState(plan.name);
            return (
              <div key={plan.name} className={`relative rounded-2xl border ${plan.popular ? "border-gold-500/50" : "border-gray-800"} bg-gray-900/50 p-8 transition-all hover:border-gold-500/30`}>
                {plan.popular && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                    <span className="bg-gradient-to-r from-gold-500 to-gold-600 text-gray-950 text-xs font-semibold px-4 py-1.5 rounded-full">Most Popular</span>
                  </div>
                )}
                ...
                <ul className="space-y-3 mb-8">
                  {plan.features.map((feature) => (
                    <li key={feature} className="flex items-start gap-3">
                      <svg className="w-� h-5 text-gold-500 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                  ...
                  {!isPaid ? (
                    ...
                    <div className="space-y-3 text-center">
                      <div className="rounded-lg bg-[#00D632]/10 border border-[#00D632]/30 p-4">
                        <p className="text-[#00D632] font-bold text-2xl">{CASHPPP_TAG}</p>
