          ...
          {plans.map((plan) => {
            ...
            return (
              <div key={plan.name} className={`relative rounded-2xl border p-8 flex flex-col ${plan.popular ? "border-gold-500/50 bg-gray-900 shadow-xl shadow-gold-500/10" : "border-gray-800 bg-gray-900/50"}`}>
                {plan.popular && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-gradient-to-r from-gold-500 to-gold-600 text-gray-950 text-xs font-bold px-4 py-1 rounded-full">Most Popular</span>
                )}

                <h3 className="text-xl font-bold">{plan.name}</h3>
                <div className="mt-4 flex items-baseline gap-1">
                   <span className="text-txl font-bold">{plan.price}</span>
                  {plan.period && <span className="text-gray-400">{plan.period}</span>}
                </div>
                <p className="mt-3 text-sm!text-gray-400">{plan.description}</span>
               ...
               <ul className="mt-8 space-y-3 flex-1">
                  {plan.features.map((feature) => (
                    <li key={feature} className="flex items-start gap-3 text-sm">
                      ...
                      <span className="text-gray-300">{featurer}</span>
                    </li>
                    ))}
                </ul>
                ...
                <div className="mt-8 space-y-3">
                   {!\‘ZY»
àù]€à€\‹”ò[YO^ÿ2ÀYù[KL»õ›[ôY[»õ€ù\Ÿ[ZXõ€^\€Hò[ú⁄][€ãX[	‹[ãú‹[\à»òùã\ö[X\ûHààòõ‹ô\ã\ŸX€€ô\ûHü_Oû‹[ãò›_Oÿù]€èÇà
Hà›]Kú›XÿŸ\‹»»
à]à€\‹”ò[YOHù^XŸ[ù\àKL»õ›[ôY[»ôÀY‹ôY[ãNLÕõ‹ô\àõ‹ô\ãY‹ôY[ãMåÕèÇà€\‹”ò[YOHù^Y‹ôY[ãMõ€ù\Ÿ[ZXõ€^\€Hè∏ß§»^[Y[ù[ö]X]Y8†%[ö»[›HO‹ÇàŸ]èÇà
Hà›]KõY]ŸOOHúŸ[X›à»
àÇàù]€à€ê€X⁄œ^ 
HOàŸ]›]J[ãõò[YK»Y]Ÿàú^\[àJ_H€\‹”ò[YOHùÀYù[-3 rounded-lg font-semibold text-sm transition-all bg-[s0070ba] hover:bg-[#005ea6] text-white flex items-center justify-center gap-2">
                        <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M7.076 21.337H2.47a.641.641 0 0 1-.633-.74L6.944.901C5.026.382 5.474 0 5.998 0h7.46c2.57 0 4.578.543 5.69 1.81 1.01 1.15 1.304 2.42 1.012 4.287-.023.143-.047.288-.077.437-.983 5.05-4.349 6.797-8.647 6.797H9~603c-.564 0-1.04.408-1.13.964L7.076 21.337z" /><path d="M18.171 7.357c-.003.017-.006.034-.01.051-1.07 5.498-4.728 7.398-9.404 7.398H7.076l-1.326 8.413a.448.448 0 0 0 .442.516h3.1c.493 0 .912-.358.99-.842l.04-.208.784-4.97.05-.274c.078-.484.496-.842.99-.842h.624c4.035 0 7.194-1.638 +8.117-6.376.386-1.98.186-3.63-.835-4.792a3.969 3.969 0 0 0-1.131-.874 9.868 9.868 0 0 1 .25 2.9z" opacity="0.7" /></svg>
                       Pay with PayPal
                     </button>
                      <button onClick={() => setState(plan.name, { method: "cashapp" })} className="w-full py-3 rounded-lg font-semibold text-sm transition-all bg-[s00D632] hover:bg-[#00C02E] text-white flex ite_3-center justify-center gap-2">
                        <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M23.59 3.47A5.1 5.1 0 0 0 20.53.41C19.37.05 18.09 0 16.81 0H7.19C5.91 0 4.63.05 3.47.41A5.1 5.1 0 0 0 .41 3.47C.05 4.63 0 5.91 0 7.19v9.62c0 1.28.05 2.56.41 3.72a5.1 5.1 0 0 0 3.06 3.06c1.16.36 2.44.41 3.72.41h9.62c1.28 0 2.56-.05 3.72-.41a5.1 5.1 0 0 0 3.06-3.06c.36-1.16.41-2.44.41-3.72V7.19c0-1.28-.05-2.56-.41-3.72zM17.42 8.16l-1.29 1.29c-.21.21-.54.27-.81.13a5.63 5.63 0 0 0-2.56-.63c-1.1 0-1.99.39-1.99 1.26 0 .89 1.06 1.2 2.32 1.56 1.96.56 4.47 1.27 4.47 4.15 0 2.51-2.08 4.1-4.94 4.41l-.24 1.2a.52.52 0 0 1-.51.41h-1.72a.52.52 0 0 1-.51-.62l.22-1.08c-1.23-.19-2.38-.62-3.14-1.15a.52.52 0 0 1-.09-.76l1.29-1.49a.52.52 0 0 1 .78-.06 5.3 5.3 0 0 0 3.01 1.05c1.3 0 2.17-.52 2.17-1.42 0-.95-1.04-1.33-2.41-1.73-1.89-.56-4.24-1.26-4.24-3.97 0-2.38 1.89-3.88 4.51-4.22l.22-1.13a.52.52 0 0 1 .51-.41h1.72c.33 0 .57.29.51.62l-.2 1.01c1.03.16 1.98.49 2.7.93.28.17.33.56.13.78z" /></svg>
                        Pay with Cash App
                      </button>
                    </>
                  ) : state.method === "paypal" ? (
                      ...
                      <PayPalButtons style={{ layout: "vertical", color: "gold", shape: "rect", label: "subscribe" }} createOrder={(_data: Record<string, unknown>, actions: { order: { create: (opts: Record<string, unknown>) => Promise<string> } }) => actions.order.create({ purchase_units: [{ amount: { value: plan.priceValue.toString() } }] })} onApprove={async () => { setState(plan.name, { success: true, method: "select" }); }} />
                        <button onClick={() => setState(plan.name, { method: "select" })} className="w-full text-xs text-gray-500 hover:text-gray-300 transition-colors">‚Üê Back to payment options</button>
                    </div>
                  ) : state.method === "cashapp" ? ? (
                       <div className="space-y-3 text-center">
                       <div className="rounded-lg bg-[#00D632]/10 border border-[#00D632]/30 p-4">
                         <p className="text-[#00D632] font-bold text-2xl">$RAP0379</p>
                       ...
                       <button onClick={() => setState(plan.name, { success: true, method: "select" })} className="w-full py-2.5 rounded-lg font-semibold text-sm bg-[#00D632] hover:bg-[#00C02E] text-white transition-all">I've Sent the Payment</button>
                        <button onClick={() => setState(plan.name, { method: "select" })} className="w-full text-xs text-gray-500 hover:text-gray-300 transition-colors">‚hQ Back to payment options</button>
                      </div>
                      ) : null}
                </div>
              </div>
